...
async function generatePDF() {
  ...
  for (var i in props.classStudentList) {
    ...
    console.log(props.classStudentList[i]["course_code"])
    ...
    pdf.setFont(undefined, 'normal');
    pdf.text(props.classStudentList[i]["subject_code"], 37, 65)
    pdf.text(props.classStudentList[i]["subject_description"], 37, 70)
    pdf.text(props.classStudentList[i]["course_code"], 37, 75)
    pdf.text(props.classStudentList[i]["section"], 37, 80)
    pdf.text(`${props.classStudentList[i]["units"]}`, 37, 85)
    pdf.text(props.userData.username, 142, 65)
    pdf.text(`${props.userData.first_name} ${props.userData.last_name}`, 142, 70)
    let upperSemester = props.classStudentList[i]["semester"].toUpperCase();
    pdf.text(upperSemester, 142, 75);
    pdf.text(props.classStudentList[i]["school_year"], 142, 80)
    ...
    const data = props.classStudentList[i]["student_list"];
